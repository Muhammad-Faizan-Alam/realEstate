...
exports.getAgent = async (req, res) => {
  try {
    const agent = await Agent.findById(req.params.id).populate('user').populate('connections');
...
exports.findAgent = async (req, res) => {
  try {
    const agent = await Agent.findOne({user: req.params.id}).populate('user').populate('connections');
    if (!agent) return res.status(404).json({ message: "Agent not found" });
