...
exports.getAllProperties = async (req, res) => {
  try {
    const properties = await PropertyDetail.find().populate('developer').populate('validatedInfo.developer').populate('regulatoryInfo.agency');
    res.json(properties);
  } catch (err) {
...
exports.getPropertyById = async (req, res) => {
  try {
    const property = await PropertyDetail.findById(req.params.id).populate('developer').populate('validatedInfo.developer').populate('regulatoryInfo.agency');
...
// get property by agency
exports.getPropertiesByAgency = async (req, res) => {
  try {
    const properties = await PropertyDetail.find({
      "regulatoryInfo.agency": req.params.id,
    })
      .populate("developer")
      .populate("validatedInfo.developer");
